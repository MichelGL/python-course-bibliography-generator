...
class TestReaders:
    ...
    def test_thesis_abstract(self, workbook: Any) -> None:
        """
        ...
        models = ThesisAbstractReader(workbook).read()

        assert len(models) == 1
        model = models[0]

        model_type = ThesisAbstractModel

        assert isinstance(model, model_type)
        assert model.author == "Иванов И.М."
        assert model.thesis_title == "Наука как искусство"
        assert model.degree == "д-р. / канд."
        assert model.field_of_science == "экон."
    ...
    def test_newspaper_article(self, workbook: Any) -> None:
        ...
        assert model.authors == "Иванов И.М., Петров С.Н."
        assert model.article_title == "Наука как искусство"
        assert model.newspaper == "Южный Урал"
